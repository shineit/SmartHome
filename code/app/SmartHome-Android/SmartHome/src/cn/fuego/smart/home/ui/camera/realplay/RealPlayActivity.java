 ...
 * @warning 这里写本文件的相关警告
 */
package cn.fuego.smart.home.ui.camera.realplay;
...
import cn.fuego.smart.home.ui.camera.util.AudioPlayUtil;
import cn.fuego.smart.home.ui.camera.util.VerifySmsCodeUtil;
import cn.fuego.smart.home.widget.HVScrollView;
...
public class RealPlayActivity extends Activity implements OnClickListener, SurfaceHolder.Callback, Handler.Callback,
    ...
    //RelativeLayout TitleBar mTitleBar = null;
    ...
    private void initView() {
        ...
        //自定义样式和监听事件
        Button back_btn= (Button) findViewById(R.id.realplay_page_back_btn);
        back_btn.setOnClickListener(this);
        mTiletRightBtn= (Button) findViewById(R.id.realplay_page_set_btn);
        mTiletRightBtn.setOnClickListener(this);
        //使用自己的titleBar
        mTitleBar = (RelativeLayout) findViewById(R.id.realplay_page_title_bar);
        mTitle = (TextView) findViewById(R.id.realplay_page_title);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.realplay_full_sound_btn:
                onSoundBtnClick();
                break;
            case R.id.realplay_page_back_btn:
                if (mStatus != RealPlayStatus.STATUS_STOP) 
                {
                    stopRealPlay();
                    setRealPlayStopUI();
                ...
                finish();
                break;
            case R.id.realplay_page_set_btn:
            	 EzvizAPI.getInstance().gotoSetDevicePage(mCameraInfo.getDeviceId(), mCameraInfo.getCameraId());
                break;
            default:
                break;
    ...
    private void initUI() {
        if (mCameraInfo != null) {
           // mTitleBar.setTitle(mCameraInfo.getCameraName());
        	mTitle.setText(mCameraInfo.getCameraName());
            ...
            if(cameraName != null) {
               // mTitleBar.setTitle(URLDecoder.decode(cameraName));
                mTitle.setText(URLDecoder.decode(cameraName));
            }
    ...
    private void startRealPlay() {
        ...
        if (mCameraInfo != null) {
            ...
            mRealPlayerHelper.startDemoRealPlayTask(mDemoRealPlayer,
                    mRtspUrl.substring(0, len == 0 ? mRtspUrl.length() : len));
        }
