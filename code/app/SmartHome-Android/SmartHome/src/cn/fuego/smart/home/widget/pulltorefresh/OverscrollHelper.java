 ...
 *******************************************************************************/
package cn.fuego.smart.home.widget.pulltorefresh;

import android.annotation.TargetApi;
import android.view.View;
import cn.fuego.smart.home.widget.pulltorefresh.IPullToRefresh.Mode;
import cn.fuego.smart.home.widget.pulltorefresh.IPullToRefresh.State;
...
public final class OverscrollHelper {
     ...
     * <p/>
     * This should only be used on AdapterView's such as ListView as it just calls through to
     * overScrollBy() with the scrollRange = 0. AdapterView's do not have a scroll range (i.e.
     * getScrollY() doesn't work).
     * 
     * @param view
     *            - PullToRefreshView that is calling this.
     * @param deltaX
     *            - Change in X in pixels, passed through from from overScrollBy call
     * @param scrollX
     *            - Current X scroll value in pixels before applying deltaY, passed through from
     *            from overScrollBy call
     * @param deltaY
     *            - Change in Y in pixels, passed through from from overScrollBy call
     * @param scrollY
     *            - Current Y scroll value in pixels before applying deltaY, passed through from
     *            from overScrollBy call
     * @param isTouchEvent
     *            - true if this scroll operation is the result of a touch event, passed through
     *            from from overScrollBy call
     */
    ...
    /**
     * Helper method for Overscrolling that encapsulates all of the necessary function. This version
     * of the call is used for Views that need to specify a Scroll Range but scroll back to it's
     * edge correctly.
     * 
     * @param view
     *            - PullToRefreshView that is calling this.
     ...
     *            - Threshold for which the values how fuzzy we should treat the other values.
     *            Needed for WebView as it doesn't always scroll back to it's edge. 0 = no
     *            fuzziness.
     * @param scaleFactor
        ...
        if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
            ...
            if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
                ...
                if (PullToRefreshBase.DEBUG) {
                    LogUtil.debugLog(LOG_TAG, "OverScroll. DeltaX: " + deltaX + ", ScrollX: " + scrollX + ", DeltaY: " + deltaY
