package cn.fuego.smart.home.ui.camera.devicelist;
 ...
 * @data 2014-4-9
 */
public class AutoWifiConnectingActivity extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setListener();
        mWaitDlg.show();
        new GetCamersInfoTask(STATUS_WIFI_CONNETCTING).execute();
    }
    ...
    private void init() {
        ...
        if(!TextUtils.isEmpty(deviceType)) {
            isSupportNetWork = DeviceUtil.isSupportNetWork(deviceType);
            isSupportWifi = DeviceUtil.isSupportWifi(deviceType);
        }
        LogUtil.debugLog(TAG, "serialNo = " + serialNo + ",mVerifyCode = " + mVerifyCode + ",wifiPassword = "
            ...
            public void run() {
                ...
                if (oneStepWifiConfigurationManager == null) {
                    ...
                    oneStepWifiConfigurationManager = new OneStepWifiConfigurationManager(
                            AutoWifiConnectingActivity.this, maskIpAddress);
                    oneStepWifiConfigurationManager.setDeviceDiscoveryListener(deviceDiscoveryListener);
                    LogUtil.debugLog(TAG, wifiSSID + " " + wifiPassword + " " + maskIpAddress);
                }
                int startSendConfigData = oneStepWifiConfigurationManager.startConfig(wifiSSID, wifiPassword);
                ...
                if (!isFinishing()
                        && ConnectionDetector.getConnectionType(AutoWifiConnectingActivity.this) == ConnectionDetector.WIFI
    ...
    private void addCameraFailed(int errorStep, int errorCode) {
        ...
        switch (errorStep) {
            ...
            case ERROR_ADD_CAMERA:
                ...
                if(errorCode == ErrorCode.ERROR_WEB_DIVICE_NOT_HAVE) {
    ...
    private class GetCamersInfoTask extends AsyncTask<Void, Void, CameraInfo> {
        private int mStatus = STATUS_WIFI_CONNETCTING;
        private int mErrorCode = 0;
        
        private GetCamersInfoTask(int status) {
            mStatus = status;
        }
            ...
            switch (errorCode) {
                ...
                default:
                    switch (mStatus) {
                        case STATUS_WIFI_CONNETCTING:
                        case STATUS_REGISTING:
                            addCameraFailed(ERROR_WIFI_CONNECT, mErrorCode);
                            break;
                        case STATUS_ADDING_CAMERA:
                        case STATUS_ADD_CAMERA_SUCCESS:
                        default:
