...
//    self loadRightCustomButtonItemWithTitle:<#(NSString *)#> image:<#(UIImage *)#>
    [self loadBackButtonItem];
    // key为空可以传个任意字符过去
    ...
    if (_enState == STATE_NONE)
    {
        ...
        if ([self.ccode.m_strAESVersion length]>5)
        {
            //支持AES加密设备
            [_simpleWifi startWifiConfig:self.strSsid addKey:self.strKey secretKey:self.ccode.m_strSN];
        }
        else
        {
            //截止app2.2版本以前的设备
...
//#endif
    ...
    if ([self.ccode.strModel length] >0 )
    {
        //扫描带设备型号
...
//    [self.view addSubview:_titleView];
    ...
    self.layer.strokeColor = [UIColor colorWithHex:0x1b9ee2].CGColor;//;UIColorFromRGB(0x1b9ee2, 1.0f).CGColor;
    ...
    if (_enState != STATE_LINE)
    {
        ...
        self.bgLayer.strokeColor = [UIColor colorWithHex:0x1b9ee2].CGColor;//UIColorFromRGB(0x1b9ee2, 1.0f).CGColor;
    ...
    tipsLab.textColor = [UIColor colorWithHex:0x444444];//UIColorFromRGB(0x444444, 1.0f);
...
- (void)addDevice
{
   ...
   withAccessToken:self.accessToken
          deviceId:self.ccode.m_strSN
           safeKey:self.ccode.strVerifyCode];//[[YSDemoDataModel sharedInstance] userAccessToken]
}
...
- (void)showFailedView:(int)nErrorCode
{
    ...
    if (!_failedView)
    {
        ...
        failedLab.textColor = [UIColor colorWithHex:0x444444];//UIColorFromRGB(0x444444, 1.0f);
        ...
        [retryBtn setTitleColor:[UIColor colorWithHex:0x40a2ee] forState:UIControlStateNormal];
...
//                [self backToEnter];
            ...
            case 2001:
        {
            if (buttonIndex == 1)
            {
                self.ccode.strVerifyCode = [alertView textFieldAtIndex:0].text;
                
                if ([self.ccode.strVerifyCode length] == 0)
                {
                    [self inputVerifyAlert];
...
- (void)finishSearchBonjourServer:(NSArray *)arrBonjourServer
{
    for (NSString * name in arrBonjourServer)
    {
        ...
        NSRange range = [name rangeOfString:self.ccode.m_strSN];
        if (range.length != 0)
        {
